...
const fetch_game = async (game_id: number): Promise<void> => {
  const url: URL = new URL(`api/search/${game_id}`,'https://jsam.qleap.jp/')
  const response = await fetch(url.href)
  console.log(`Fetched game ${game_id} status: ${response.status}`)
}

export async function fetchAll() {
  const games: number[][] = chunk(Array.from({ length: 200 }, (_, i) => i), 25);
